...
if parallelFlag
   ...
   addAttachedFiles(p,fullfile(rootDir,{'3d sim','Libraries'}));
    parfevalOnAll(@addpath,0,fullfile(rootDir,'3d sim'), ... 
    ...
    cd(fullfile(rootDir,'3d sim'));
    if exist('temp','dir')
        rmdir('temp','s');
    end
    mkdir('temp');
    spmd
        tempFolder = fullfile(rootDir,'3d sim','temp');
        cd(tempFolder);
